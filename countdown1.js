(function (root, factory) {
  ...
  var init = function () {
    ...
    // $("body").append('<style>[data-wwclass="countdown1"] .aaa{display:none;}</style>');
   ...
   */
  function count_down($ele) {
    var config = {};
    config.countdownid = $ele.attr("data-countdownid");

    $ele.find(".countdown").attr("id", config.countdownid);

    var oCountDown = document.getElementById(config.countdownid);
    var timer = null;
    ...
    function updateTime() {
      ...
      $.wwclass.helper.updateProp($ele, "data-x-counttime", oRemain)
      if (oRemain <= 60 && oRemain >= 60) {
        $.wwclass.helper.anijsTrigger($ele, "counttime");
      } else if (oRemain <= 0) {
        clearInterval(timer);
        $.wwclass.helper.anijsTrigger($ele, "counttimeend");
        return
      }
      oRemain--;
